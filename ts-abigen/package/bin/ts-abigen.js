...
const ArgumentParser = __importStar(require("argparse"));
...
const abiGenMeta = JSON.parse(FS.readFileSync(Path.join(root, 'package.json')).toString());
...
const abiFiles = Object.keys(contracts).reduce((acc, contract) => {
    ...
    const outFile = Path.join(abiTempDir, contract.split(':')[1] + '.json');
...
const partialArg = Path.join(Path.join(templatePath, 'partials'), '*.handlebars');
...
contractFileNames.forEach(filename => {
    ...
    const sourceFile = TS.createSourceFile(contractFile, FS.readFileSync(contractFile).toString(), TS.ScriptTarget.ES2015, true);
...
// @ts-ignore
tsConfig.typeRoots = [].concat.apply([], require.resolve.paths('').map((path) => {
    return [
